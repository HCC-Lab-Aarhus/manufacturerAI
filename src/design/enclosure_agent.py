...
class Enclosure3DAgent:
    ...
    def generate_from_pcb_layout(
        ...
        # Note: IR diode slits are in top shell (diodes point outward from top edge)
        print("[ENCLOSURE] PATH: Generating OpenSCAD files...")
        top_scad = self._generate_top_shell_scad(button_holes, [], ir_diodes, battery_cavity)
    ...
    def _generate_top_shell_scad(
        ...
        """Generate OpenSCAD code for top shell with walls, button holes, IR slits, battery guards, and snap-fit clips."""
        p = self.params
...
// Includes snap-fit clips to click onto bottom shell
...
shell_height = {p.shell_height:.2f};  // Height of top shell side walls (fits AAA battery)

// Battery guard parameters
...
"""
    ...
    def _generate_battery_guards_scad(self, battery_cavity: Optional[Dict[str, float]]) -> str:
        """Generate OpenSCAD code for battery guards on the top shell.
        
        Creates 4 walls forming a rectangle to hold the batteries.
        The guards are slightly larger than the battery compartment opening
        in the bottom shell, and as tall as the surrounding walls.
        """
        ...
        # Guard dimensions - slightly larger than opening for overlap
        guard_width = width + 2 * p.battery_guard_wall  # Add wall thickness on each side
        guard_height = height + 2 * p.battery_guard_wall
        wall = p.battery_guard_wall
        guard_z_height = p.shell_height - p.top_thickness  # Full height minus top plate
        
        # Left wall
        lines.append(f"    translate([{cx - guard_width/2:.2f}, {cy - guard_height/2:.2f}, top_thickness])")
        lines.append(f"        cube([{wall:.2f}, {guard_height:.2f}, {guard_z_height:.2f}]);")
        
        # Right wall  
        lines.append(f"    translate([{cx + guard_width/2 - wall:.2f}, {cy - guard_height/2:.2f}, top_thickness])")
        lines.append(f"        cube([{wall:.2f}, {guard_height:.2f}, {guard_z_height:.2f}]);")
        ...
        # Back wall (top Y)
        lines.append(f"    translate([{cx - guard_width/2:.2f}, {cy + guard_height/2 - wall:.2f}, top_thickness])")
        lines.append(f"        cube([{guard_width:.2f}, {wall:.2f}, {guard_z_height:.2f}]);")
        
        return "\n".join(lines)
    
    def _generate_snap_clips_scad(self) -> str:
        """Generate OpenSCAD code for snap-fit clips on top shell walls.
        
        Creates small protruding hooks on the inside of the top shell walls
        that clip onto the rim of the bottom shell.
    ...
    def _generate_snap_rim_scad(self) -> str:
        """Generate OpenSCAD code for snap-fit rim on bottom shell.
        
        Creates a raised rim around the edge of the bottom shell that the
        top shell's clips snap onto.
        """
        p = self.params
        lines = ["            // Snap-fit rim around edge"]
        
        rim_height = 3.0
        rim_thickness = 1.2
        
        # Rim goes around the perimeter, just inside the outer edge
        # The rim is where the top shell's inner wall slides over
        rim_inset = p.wall_thickness + 0.3  # Slight clearance for top shell walls
        
        # Left rim
        lines.append(f"            translate([{rim_inset:.2f}, {rim_inset:.2f}, {p.bottom_thickness:.2f}])")
        lines.append(f"                cube([{rim_thickness:.2f}, {p.outer_length - 2*rim_inset:.2f}, {rim_height:.2f}]);")
        
        # Right rim
        lines.append(f"            translate([{p.outer_width - rim_inset - rim_thickness:.2f}, {rim_inset:.2f}, {p.bottom_thickness:.2f}])")
        lines.append(f"                cube([{rim_thickness:.2f}, {p.outer_length - 2*rim_inset:.2f}, {rim_height:.2f}]);")
        ...
        # Back rim
        lines.append(f"            translate([{rim_inset:.2f}, {p.outer_length - rim_inset - rim_thickness:.2f}, {p.bottom_thickness:.2f}])")
        lines.append(f"                cube([{p.outer_width - 2*rim_inset:.2f}, {rim_thickness:.2f}, {rim_height:.2f}]);")
    ...
    def _generate_bottom_shell_scad(
        ...
        """Generate OpenSCAD code for bottom shell with rim for snap-fit.
        
        The bottom shell is a flat plate with:
        - Trace channels carved into the top surface for conductive filament
        - Pinholes for component pins
        - Battery compartment cutout with ledges
        - A rim around the edge for the top shell to snap onto
        """
        p = self.params
        ...
        # Generate snap-fit rim
        snap_rim_code = self._generate_snap_rim_scad()
        
        scad = f"""// Bottom Shell - Generated by ManufacturerAI
// This file is parametric - edit values below to customize
// Flat base plate with rim for snap-fit connection to top shell.
// Trace channels carved into the top surface are meant to be filled
...
// Snap-fit rim parameters
rim_height = 3.0;  // Height of rim that top shell clips onto
rim_thickness = 1.2;  // Thickness of rim wall

// Main shell (flat base with snap-fit rim)
module bottom_shell() {{
    difference() {{
        union() {{
            ...
            hull() {{
                for (x = [corner_radius, outer_width - corner_radius])
                    for (y = [corner_radius, outer_length - corner_radius])
                        translate([x, y, 0])
                            cylinder(r=corner_radius, h=bottom_thickness, $fn=32);
            }}
            
            // Snap-fit rim around the edge
{snap_rim_code}
        }}
...
{battery_cutout_code}
        ...
        // Trace channels carved into top surface (for conductive filament)
{trace_channel_code}
