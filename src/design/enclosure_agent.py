...
class Enclosure3DAgent:
    ...
    def generate_from_pcb_layout(
        ...
        # Note: LED windows not included in top shell - IR diodes have slits in bottom shell only
        print("[ENCLOSURE] PATH: Generating OpenSCAD files...")
        top_scad = self._generate_top_shell_scad(button_holes, [])
...
"""
    ...
    def _generate_battery_cutout_scad(self, battery_cavity: Optional[Dict[str, float]]) -> str:
        ...
        1. Bottom ledge (outside) - wider, where the hatch panel rests
        2. Main cutout - through hole for battery access
        3. Top catch ledge (inside) - smaller ledge for the spring latch to hook onto
        
        The hatch inserts from below, the panel rests on the bottom ledge,
        and the spring latch wraps around to catch on the top ledge.
        """
        ...
        top_catch_width = 1.5  # Smaller ledge on inside for latch to catch
        bottom_ledge_depth = hatch_thickness + 0.3  # Recess depth for hatch panel
        top_catch_depth = 1.5  # Depth of catch ledge
        
        # Main through-hole dimensions (smaller than both ledges)
        ...
        # 2. Bottom ledge recess (on outside/bottom - where hatch rests)
        # This is cut from z=0 up to bottom_ledge_depth
        lines.append(f"        // Bottom ledge (outside) - hatch panel rests here")
        lines.append(f"        translate([{cx - width/2:.2f}, {cy - height/2:.2f}, -1])")
        lines.append(f"            cube([{width:.2f}, {height:.2f}, {bottom_ledge_depth + 1:.2f}]);")
        
        # 3. Top catch ledge recess (on inside/top - where spring latch hooks)
        # This is cut from top of bottom_thickness down by top_catch_depth
        # But narrower so there's material for the latch to catch on
        catch_recess_width = width - 2 * top_catch_width
        catch_recess_height = height - 2 * top_catch_width
        lines.append(f"        // Top catch area (inside) - spring latch hooks here")
        lines.append(f"        translate([{cx - catch_recess_width/2:.2f}, {cy - catch_recess_height/2:.2f}, bottom_thickness - {top_catch_depth:.2f}])")
        lines.append(f"            cube([{catch_recess_width:.2f}, {catch_recess_height:.2f}, {top_catch_depth + 1:.2f}]);")
    ...
    def _generate_battery_hatch_scad(self, battery_cavity: Dict[str, float]) -> str:
        ...
        - A spring loop on one end for latching (loop of material that flexes)
        - The loop extends outward then curves back to form a catch
        """
        p = self.params
...
// The spring loop provides retention force.
...
module spring_loop() {{
    // Spring loop: extends out, curves, comes back with a catch
    // Uses rectangular cross-section throughout (cubes for arms, curved extrusion for bend)
    ...
    // Outward arm (goes up from hatch)
    translate([0, 0, 0])
        cube([loop_width, loop_thickness, loop_height]);
    
    // Curved top connecting the two arms - rectangular cross-section swept through 180 degrees
    ...
    // Return arm (comes back down, offset by arm_gap)
    translate([0, loop_thickness + arm_gap, loop_thickness])
        cube([loop_width, loop_thickness, loop_height - loop_thickness]);
    
    // Catch/hook at the bottom of return arm
    translate([0, loop_thickness + arm_gap, loop_thickness])
        cube([loop_width, loop_thickness * 2, loop_thickness]);
}}

module battery_hatch() {{
    // Main hatch body with lip
    difference() {{
        // Outer lip (larger, sits on ledge)
        cube([hatch_width, hatch_height, hatch_thickness]);
        
        // Nothing to subtract for now (solid panel)
    }}
    
    // Spring latch on one end (centered on short edge)
    // Positioned so loop extends outward from hatch edge
    translate([(hatch_width - loop_width) / 2, -(loop_thickness * 4), 0])
        spring_loop();
    
    // Optional: finger grip notch on opposite end
