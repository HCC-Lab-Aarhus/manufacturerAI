...
def collect_and_generate_extras(
    ...
    """Collect all extra parts and generate a standalone SCAD string.

    Extra parts (buttons, hatches, etc.) are laid out in a row starting
    near the origin so they can be compiled into a separate STL and
    sliced independently from the enclosure.

    Returns a complete SCAD string for the extras file, or ``""`` if
    there are no extras.
    """
    ...
    parts: list[str] = []
    parts.append("// ============================================================")
    parts.append("// Extra parts — separate print (buttons, hatches, etc.)")
    parts.append("// Generated by ManufacturerAI")
    parts.append("// ============================================================")
    parts.append("")
    ...
    # Lay out extras in a row starting near the origin
    current_x = 0.0

    for extra in extras:
        place_x = current_x + extra.footprint_x / 2
        place_y = extra.footprint_y / 2
    ...
    log.info("Extra parts: %d generated (separate file)", len(extras))
    return "\n".join(parts)
